...
export function resolveFlamecastBaseUrl(options: ResolveBaseUrlOptions = {}) {
  const envOrigin =
    options.envOrigin ?? import.meta.env.VITE_FLAMECAST_API_ORIGIN
  if (envOrigin) {
...
export type CreateSessionRequest = NonNullable<
  Parameters<SessionsRoute['$post']>[0]
>['json']
export type SessionDetailsResponse = InferResponseType<SessionRoute['$get'], 200>
export type RunCommandResponse = InferResponseType<
  SessionRoute['exec']['$post'],
  200
>
