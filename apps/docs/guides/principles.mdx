...
This works out of the box — in-memory storage, local process runtime, built-in agent templates. But every piece is replaceable. Swap in Postgres storage, add a custom runtime provider, register your own templates, or use the `fetch` handler for serverless deployments.
