...
The reference server in `apps/server` is intentionally minimal. It exports a configured `Flamecast` instance with sensible defaults, calls `init()`, and hands that instance to `listen()`. That's it.
