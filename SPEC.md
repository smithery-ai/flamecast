...
**Constraint:** Runtime truth (subprocesses, streams, pending promises) cannot live in a database; only **projections** and **history** can.
...
- Define a narrow **`Projection`** (or **`DurableSink`**) interface that Flamecast calls on lifecycle and log paths, e.g. `appendLog`, `upsertConnectionMeta`, `setPendingPermission` / `clearPendingPermission`, `onConnectionClosed`.
- **Server wiring** (`src/server/…`) constructs `Flamecast` with an implementation: in-memory-only, SQLite, or later **`ConvexProjection`** that forwards to Convex mutations.
...
- It speaks **ACP only** to Flamecast over the transport (stdio, TCP to a sidecar, etc.). Flamecast observes the protocol, applies policy, and **writes the projection** via the persistence port.
...
flowchart TB
    ...
    FC["Flamecast + injected Projection"]
...
**Pitfall:** Deferring the **`Projection` interface** until the end forces a large retrofit through every log/lifecycle path. Cheap mitigation: add **Phase 2’s** interface with a **no-op** (or tiny JSONL) while still in **Phase 1** if convenient; flesh out storage in Phase 2 and swap **Phase 3**’s adapter when ready.
...
- **API:** `GET /connections/:id` merges **projection** (history, metadata) with **runtime** (live pending permission, “actually connected”) where both exist.
...
**Goal:** Hosted sync, subscriptions, or ops simplicity—only when the product needs it. Requires **Phase 2’s** projection port (or a no-op you already added) so the swap is an adapter only.

- Implement **`ConvexProjection`** (or HTTP calls to Convex mutations from Node) as the only new piece; **Flamecast code stays the same** aside from already using `Projection`.
...
**Rule:** Convex holds **durable projection**; Flamecast owns **runtime**. Do not treat “subprocess alive” in Convex as authoritative without clear fields (`last_seen_at`, `runtime_generation`, `status: historical | live`).
...
- [ ] Whether log bodies are capped or paged in the projection.
